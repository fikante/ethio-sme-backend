...
export default function ApplyModal({ isOpen, onClose, userName, initialSuccess = false }: Props) {
    ...
    const inputClass =
        'w-full rounded-xl border border-white/30 bg-black px-4 py-3 text-sm text-white ' +
        'placeholder:text-white/40 focus:border-white focus:outline-none focus:ring-0';
    const labelClass =
        'mb-1.5 block text-xs font-semibold uppercase tracking-wide text-white/60';
    const ghostBtn =
        'rounded-xl border border-transparent px-5 py-2.5 text-sm font-medium text-white/70 ' +
        'transition hover:border-white/30 hover:text-white';
    const primaryBtn =
        'inline-flex items-center gap-2 rounded-xl border border-white bg-white px-6 py-2.5 ' +
        'text-sm font-semibold text-black hover:bg-white/90 disabled:opacity-60';
    ...
    return (
        <>
            ...
            >
                ...
                >
                    <div className="overflow-hidden rounded-3xl border border-white/20 bg-black text-white">
                        {step !== 'success' && (
                            <>
                            ...
                            {step === 'success' ? (
                                ...
                                <>
                                    <div className="mb-4 flex items-center justify-between">
                                        ...
                                        <button type="button" onClick={onClose} className="rounded-lg p-2 text-white/60 transition hover:text-white"><X className="h-5 w-5" /></button>
                                    </div>
                                    ...
                                    {step === 3 && <Step3 data={data} ghostBtn={ghostBtn} primaryBtn={primaryBtn} onChange={update} onBack={() => setStep(2)} onNext={() => setStep(4)} />}
                                    {step === 4 && (
                                        <Step4
            ...
            <style>{`
                ...
                input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 20px; height: 20px; border-radius: 50%; background: #ffffff; cursor: pointer; border: 2px solid #000000; }
                input[type=range]::-webkit-slider-runnable-track { height: 6px; border-radius: 9999px; background: rgba(255,255,255,0.25); }
                input[type=range]::-moz-range-thumb { width: 20px; height: 20px; border-radius: 50%; background: #ffffff; cursor: pointer; border: 2px solid #000000; }
                input[type=range]::-moz-range-track { height: 6px; border-radius: 9999px; background: rgba(255,255,255,0.25); }
            `}</style>
        </>
    );
}
                    ...
                    return (
                        <div key={label} className="flex flex-1 flex-col items-center">
                            <div className="flex w-full items-center">
                                <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold ${done ? 'border border-white bg-white text-black' : active ? 'border-2 border-white bg-black text-white' : 'border border-white/30 bg-black text-white/50'}`}>
                                    {done ? <Check className="h-4 w-4" /> : n}
                                </div>
                                {n < 4 && <div className={`mx-1 h-px flex-1 ${n < current ? 'bg-white' : 'bg-white/20'}`} />}
                            </div>
                            <span className={`mt-2 text-[10px] uppercase tracking-wider ${active ? 'font-semibold text-white' : 'text-white/50'}`}>{label}</span>
                        </div>
                    );
                })}
            </div>
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-center py-2">
                <div className="flex h-16 w-16 items-center justify-center rounded-full border border-white bg-white text-xl font-bold text-black">{getInitials(data.fullName)}</div>
            </div>
            <div>
                <label className={labelClass} htmlFor="fullName">Full Name</label>
                <input id="fullName" className={inputClass} value={data.fullName} onChange={(e) => onChange('fullName', e.target.value)} />
                {errors.fullName && <p className="mt-1 text-xs text-white/80">{errors.fullName}</p>}
            </div>
            <div>
                ...
                {errors.phone && <p className="mt-1 text-xs text-white/80">{errors.phone}</p>}
            </div>
    ...
    return (
        <div className="space-y-4">
            <div>
                ...
                {errors.businessName && <p className="mt-1 text-xs text-white/80">{errors.businessName}</p>}
            </div>
            <div>
                <span className={labelClass}>Sector</span>
                {errors.sector && <p className="text-xs text-white/80">{errors.sector}</p>}
                <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3">
                    {SECTORS.map((s) => (
                        <button key={s.code} type="button" onClick={() => onChange('sector', s.code)}
                            className={`flex flex-col items-center gap-1 rounded-xl border p-3 text-xs font-medium transition-all ${data.sector === s.code ? 'border-white bg-white text-black' : 'border-white/30 bg-black text-white/70 hover:border-white'}`}>
            ...
            <div>
                ...
                {errors.subCity && <p className="mt-1 text-xs text-white/80">{errors.subCity}</p>}
            </div>
            <div>
                ...
                {errors.establishedYear && <p className="mt-1 text-xs text-white/80">{errors.establishedYear}</p>}
            </div>
...
function Step3({ data, ghostBtn, primaryBtn, onChange, onBack, onNext }: {
    data: FormState; ghostBtn: string; primaryBtn: string;
    onChange: <K extends keyof FormState>(key: K, value: FormState[K]) => void;
    onBack: () => void; onNext: () => void;
}) {
    return (
        <div className="space-y-5 text-center">
            <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-white/30 bg-black">
                <Shield className="h-8 w-8 text-white" />
            </div>
            <div>
                <h3 className="text-lg font-semibold text-white">Creditworthiness Assessment</h3>
                <p className="mx-auto mt-2 max-w-sm text-sm text-white/60">This test evaluates your integrity, conscientiousness, and risk tolerance. It takes about 3 minutes.</p>
            </div>
            <div className="flex flex-wrap justify-center gap-2">
                {['🤝 Integrity', '📊 Conscientiousness', '⚖️ Risk Tolerance'].map((pill) => (
                    <span key={pill} className="rounded-full border border-white/30 px-3 py-1 text-xs text-white/70">{pill}</span>
                ))}
            </div>
            <button type="button" onClick={() => window.open(route('psychometric-test'), '_blank')} className="flex w-full items-center justify-center gap-2 rounded-xl border border-white bg-white py-4 text-sm font-semibold text-black hover:bg-white/90">
                <Brain className="h-5 w-5" />Take Psychometric Test
            </button>
            <label className="flex items-center justify-center gap-2 text-sm text-white/60">
                <input type="checkbox" checked={data.psychometricDone} onChange={(e) => onChange('psychometricDone', e.target.checked)} className="rounded border-white/40 bg-black text-white focus:ring-0" />
        ...
        <div className="space-y-5">
            <div>
                ...
                <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} onClick={() => fileInputRef.current?.click()}
                    className={`mt-2 cursor-pointer rounded-2xl border-2 border-dashed p-8 text-center transition-colors ${dragOver ? 'border-white bg-black' : 'border-white/30 bg-black'}`}>
                    <FileUp className="mx-auto h-8 w-8 text-white/60" />
                    <p className="mt-3 text-sm font-medium text-white">Drag & drop your file here</p>
                    <p className="mt-1 text-xs text-white/50">or click to browse</p>
                    <p className="mt-2 text-xs text-white/50">Accepted: .csv, .xlsx — Max 10MB</p>
                ...
                {data.transactionFile && (
                    <div className="mt-2 flex items-center gap-2 text-sm text-white">
                        ...
                        <button type="button" onClick={onRemoveFile} className="text-xs text-white/60 underline">Remove</button>
                    </div>
                )}
                {errors.transaction_file && <p className="mt-1 text-xs text-white/80">{errors.transaction_file}</p>}
            </div>
            <div>
                <span className={labelClass}>How much do you need? (ETB)</span>
                <p className="my-2 text-center text-3xl font-bold tabular-nums text-white">{formatEtb(data.requestedAmount)}</p>
                ...
                <div className="mt-2 grid grid-cols-4 gap-2">
                    {TENURE_OPTIONS.map((months) => (
                        <button key={months} type="button" onClick={() => onChange('tenureMonths', months)}
                            className={`rounded-xl border py-3 text-xs font-semibold sm:text-sm ${data.tenureMonths === months ? 'border-white bg-white text-black' : 'border-white/30 bg-black text-white/70 hover:border-white'}`}>
                            {months} mo
                        </button>
                    ))}
                </div>
            ...
            <div>
                ...
                <p className="mt-1 text-right text-xs text-white/50">{data.purpose.length} / 500</p>
                {errors.purpose && <p className="mt-1 text-xs text-white/80">{errors.purpose}</p>}
            </div>
            <button
                type="button"
                onClick={onSubmit}
                disabled={submitting}
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-white bg-white py-5 text-base font-semibold text-black hover:bg-white/90 disabled:opacity-60"
            >
...
function SuccessView({ onClose }: { onClose: () => void }) {
    ...
    return (
        <div className="py-6 text-center">
            <div className="check-bounce mx-auto flex h-20 w-20 items-center justify-center rounded-full border border-white/30 bg-black">
                <CheckCircle className="h-10 w-10 text-white" />
            </div>
            <h2 className="mt-4 text-2xl font-bold text-white">Application Submitted!</h2>
            <p className="mx-auto mt-2 max-w-sm text-sm text-white/60">Your application is queued for AI evaluation. The loan officer will review your results shortly.</p>
            <div className="mt-8 flex flex-wrap items-center justify-center gap-2 text-xs">
                {steps.map((s, i) => (
                    <div key={s.label} className="flex items-center gap-2">
                        <span className={s.done ? 'font-semibold text-white' : 'text-white/40'}>{s.done ? '✓' : '○'} {s.label}</span>
                        {i < steps.length - 1 && <span className="text-white/30">────</span>}
                    </div>
                ))}
            </div>
            <div className="mt-8 flex flex-col items-center gap-3">
                <button type="button" onClick={onClose} className="rounded-xl border border-white bg-white px-8 py-3 text-sm font-semibold text-black hover:bg-white/90">Close</button>
                <Link href={route('sme.valuation')} className="text-sm font-medium text-white hover:underline">View Status →</Link>
            </div>
        </div>
    );
}
